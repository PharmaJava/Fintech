...
import { getLocalizedPost } from '@/content/blog';
...
export function BlogPostPage() {
  ...
  const post = slug ? getLocalizedPost(slug, getLocale()) : undefined;
  ...
  return (
    <article className="mx-auto w-full max-w-2xl px-4 py-12">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        {formatDate(post.date)} · {post.readingMinutes} {t('blog.minutes')}
      </p>
      ...
      <div className="mt-10 rounded-lg border bg-primary/5 p-6 text-center">
        <p className="font-semibold">{t('blog.cta.title')}</p>
        <Link
          to="/app"
